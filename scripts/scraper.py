...
HEADERS = {
    "User-Agent": "BeybladeX-Database-Scraper/1.0 (educational/research project)"
}
...
CANONICAL_BLADES = {
    # BX Series
    "Dran Sword", "Hells Scythe", "Knight Shield", "Knight Lance", "Wizard Arrow",
    "Shark Edge", "Viper Tail", "Dran Dagger", "Hells Hammer", "Leon Claw",
    "Dran Buster", "Hells Chain", "Rhino Horn", "Black Shell", "Tyranno Beat",
    "Unicorn Sting", "Phoenix Wing", "Wizard Rod", "Aero Pegasus", "Silver Wolf",
    "Shark Slayer", "Samurai Saber", "Weiss Tiger", "Talon Ptera", "Spike Cadeus",
    "Roar Tyranno",
    # CX Series
    ...
    "Blast Emperor", "Emperor Brave", "Dran Brave", "Might Blast", "Hover Wyvern",
    "Shark Scale", "Meteor Dragoon", "Impact Drake", "Rock Golem", "Samurai Calibur",
    "Crimson Garuda", "Scorpio Spear", "Shinobi Shadow", "Ghost Circle", "Bite Croc",
    "Keel Shark", "Knife Shinobi", "Shelter Drake", "Sphinx Cowl", "Tide Whale",
    "Tricera Press", "Bear Scratch", "Chain Incendio", "Clock Mirage", "Dragoon Storm",
    "Driger Slash", "Gill Shark", "Mummy Curse", "Scythe Incendio", "Sting Unicorn",
    "Wand Wizard", "Xeno Xcalibur", "Cerberus Blast",
}
...
KNOWN_ASSISTS = {
    "Jaggy", "Slash", "Wheel", "Bumper", "Heavy", "Assault", "Rush", "Low",
}
...
def split_blade_assist(blade_text: str) -> tuple[str, Optional[str]]:
    ...
    is_assist = (
        last_word in ASSIST_ABBREVIATIONS or
        last_word.lower() in KNOWN_ASSISTS_LOWER or
        (len(last_word) <= 2 and last_word.isupper())  # Single/double letter uppercase
    )
...
def parse_combo(combo_str: str) -> Optional[Combo]:
    ...
    combo_str = re.sub(r'\s*\([^)]*(?:Stage|Finals|Only|Match|Type)[^)]*\)', '', combo_str, flags=re.I)
    ...
    match = re.match(
        r'^(.+?)\s+(\d{1,2}-\d{2,3})\s+([A-Za-z][A-Za-z\s]*)$',
        combo_str
    )
    if match:
        ...
        return Combo(blade=blade, ratchet=ratchet, bit=bit, assist=assist, lock_chip=lock_chip)

    # Try: Everything + Ratchet+Bit (no space, bit attached like 3-60F or 6-60V)
    match = re.match(
        r'^(.+?)\s+(\d{1,2}-\d{2,3})([A-Z][A-Za-z]*)$',
        combo_str
    )
    if match:
        blade_part = match.group(1).strip()
...
def parse_date(date_str: str) -> Optional[datetime]:
    ...
    for fmt in formats:
        try:
            ...
            # Reject dates more than 1 month in the future (results are always posted after)
            if parsed > now + timedelta(days=30):
                # For 2-digit year formats, try adjusting century
                if fmt == "%m/%d/%y" and parsed.year > now.year:
                    # e.g., "1/27/27" parsed as 2027 should probably be 2025
                    # Try subtracting years until reasonable
                    adjusted = parsed.replace(year=parsed.year - 100)
                    if adjusted.year >= 2023:  # BeybladeX started ~2023
                        return adjusted
                continue
            return parsed
        except ValueError:
...
def parse_location(text: str) -> tuple[Optional[str], Optional[str], Optional[str]]:
    ...
    text = re.sub(r'\b(X Format|Ranked|Unranked|1on1|3on3|1v1|3v3|Experimental|Beyblade X)\b', '', text, flags=re.I)
    text = text.strip(' -,')
...
def parse_header_lines(lines: list[str]) -> dict:
    ...
    for line in header_lines:
        ...
        if re.match(r'^[\d/]+$', line_clean) or re.match(r'^[A-Z][a-z]+ \d{1,2},? \d{4}$', line_clean):
            continue

        # Skip lines that are just format indicators
        if re.match(r'^(Beyblade X|X Format|Ranked|Unranked|1on1|3on3)$', line_clean, re.I):
            continue
        ...
        # Skip common noise
        if line_clean.lower() in ['winning combos', 'top 3 photo', '(click to view)', 'top 3 deck combos', '!']:
            continue

        # Check if this line looks like a location (has commas and country-like words)
        has_location_pattern = bool(re.search(r',\s*(Canada|USA|US|UK|Japan|Australia|Germany|France)', line_clean, re.I))
        has_date_in_line = bool(re.search(r'\d{1,2}/\d{1,2}/\d{2,4}|[A-Z][a-z]+ \d{1,2},? \d{4}', line_clean))
        ...
        if has_location_pattern or (has_date_in_line and ',' in line_clean):
            ...
            name = re.sub(r'\s*[-|]\s*(X Format|Ranked|Unranked|1on1|3on3).*$', '', name, flags=re.I)
...
def parse_post(post_element) -> list[Tournament]:
    ...
    for i, line in enumerate(lines):
        ...
        has_date = re.search(r'\d{1,2}/\d{1,2}/\d{2,4}', line) or re.search(r'[A-Z][a-z]+ \d{1,2},? \d{4}', line)
        ...
        if has_date and tournament_created and current_tournament:
            ...
            is_header_line = (
                line.strip().startswith('-') or  # "- 07/29/23"
                re.match(r'^[A-Z][a-z]+ \d{1,2},? \d{4}$', line.strip()) or  # Just date
                re.match(r'^\d{1,2}/\d{1,2}/\d{2,4}$', line.strip())  # Just date
            )
        ...
        place_match = re.match(r'^(1st|2nd|3rd)\s*(Place)?[:\s-]*(.*)$', line, re.IGNORECASE)
        if place_match:
        ...
        if current_place is not None:
            ...
            if current_player is None and not re.search(r'\d-\d{2}', line):
                ...
                noise_patterns = [
                    '!', '(Click to View)', 'WINNING COMBOS', 'Top 3 Photo', 'Top 3 Deck Combos',
                    'Both Stages', 'First Stage', 'Final Stage', 'Finals Only', 'First Stage Only',
                    '3on3 Match Finals Only', '3on3 Finals Only', 'Match Finals Only',
                ]
...
def get_processed_post_ids(conn) -> set[str]:
    ...
    result = conn.execute("SELECT wbo_post_id FROM tournaments WHERE wbo_post_id IS NOT NULL").fetchall()
...
def scrape_all(max_pages: Optional[int] = None, delay: float = 1.0, fresh: bool = False):
    """
...
def show_stats():
    ...
    print(f"\n=== TOP 10 BLADES ===")
    for row in conn.execute("SELECT part_name, total_placements, win_rate FROM part_stats WHERE part_type = 'blade' ORDER BY total_placements DESC LIMIT 10").fetchall():
        print(f"  {row[0]}: {row[1]} uses, {row[2]:.1%} win rate")

    print(f"\n=== TOP 10 COMBOS ===")
    for row in conn.execute("SELECT combo, total_placements, win_rate FROM combo_stats ORDER BY total_placements DESC LIMIT 10").fetchall():
        print(f"  {row[0]}: {row[1]} uses, {row[2]:.1%} win rate")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if cmd == "test":
            ...
            print("  python scraper.py N       - Scrape N pages (incremental)")
            print("  python scraper.py fresh N - Fresh scrape N pages (clears existing)")
    else:
        # Default: incremental scrape of 5 pages
