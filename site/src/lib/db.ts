...
//
// SOURCE OF TRUTH: scripts/scrapers/fandom.py populates parts_catalog table from
// the Fandom wiki Category pages, then catalog.py exports parts_catalog.json into
// site/public/data/. initDB() below fetches that JSON and *overlays* its entries
// onto the maps in this file. Adding a new wiki part = zero edits in this file.
//
// The hardcoded entries below remain as a safety net so the site renders something
// useful before the catalog loads (or if the JSON is unavailable).
export let BLADE_SERIES: Record<string, 'BX' | 'CX' | 'UX'> = {
  // ==========================================================================
...
// Mutable — initDB() overlays new entries from parts_catalog.json.
let CX_BLADES_REQUIRING_LOCKCHIP = new Set([
  'Brave', 'Arc', 'Dark', 'Reaper', 'Brush', 'Blast',
  'Eclipse', 'Hunt', 'Might', 'Flare', 'Flame', 'Fang', 'Fort',
  'Antler', 'Wriggle', 'Volt'
]);

// Metal lock chips (all others are plastic — lock chip identity doesn't affect
// competitive performance). Mutable — initDB() overlays from catalog.
let METAL_LOCK_CHIPS = new Set(['Emperor', 'Valkyrie']);
...
// Bit display names — abbreviations and dirty-data variants to canonical names.
// Mutable — initDB() overlays the bit_aliases map from parts_catalog.json.
let BIT_DISPLAY_NAMES: Record<string, string> = {
 ...
 *
 * This file is generated by scripts/catalog.py:export_catalog_json after
 * every fandom scrape. It's the single source of truth — adding a new wiki
 * part = re-run the fandom scrape and the frontend picks it up automatically.
 */
async function loadCatalogJson(): Promise<void> {
  type CatalogJson = {
    lock_chips?: Array<{ name: string; metal: boolean }>;
    main_blades?: string[];
    metal_blades?: string[];
    over_blades?: string[];
    assists?: string[];
    bits?: string[];
    ratchets?: string[];
    blades?: Array<{ name: string; system: 'BX' | 'UX' | 'CX' }>;
  ...
  const res = await fetch(`/data/parts_catalog.json?v=${cacheBuster}`, { cache: 'no-store' });
  if (!res.ok) throw new Error(`HTTP ${res.status}`);
  const cat = (await res.json()) as CatalogJson;

  // BLADE_SERIES: BX/UX standalone blades + every CX full name
  if (cat.blades) {
    for (const { name, system } of cat.blades) {
      BLADE_SERIES[name] = system;
    }
  }
  if (cat.cx_full_names) {
    for (const name of cat.cx_full_names) {
      BLADE_SERIES[name] = 'CX';
    }
  }
  // CX main blades (require lock chip prefix)
  if (cat.main_blades) {
    CX_BLADES_REQUIRING_LOCKCHIP = new Set(cat.main_blades);
    for (const name of cat.main_blades) {
      BLADE_SERIES[name] = 'CX';
    }
  }
  if (cat.metal_blades) {
    for (const name of cat.metal_blades) {
      BLADE_SERIES[name] = 'CX';
    }
  }

  // METAL_LOCK_CHIPS
  if (cat.metal_lock_chips) {
    METAL_LOCK_CHIPS = new Set(cat.metal_lock_chips);
  }
  ...
  console.log(
    `[Catalog] Loaded ${Object.keys(BLADE_SERIES).length} blades, ` +
    `${METAL_LOCK_CHIPS.size} metal lock chips, ` +
    `${Object.keys(BIT_DISPLAY_NAMES).length} bit aliases`
  );
}
...
export async function initDB(): Promise<duckdb.AsyncDuckDBConnection> {
  ...
  initPromise = (async () => {
    ...
    try {
      ...
      // Fetch the wiki-derived parts catalog and merge it into the in-memory
      // lookup tables. This is the source of truth for valid parts; the
      // hardcoded entries above are only a safety net.
      try {
        await loadCatalogJson();
      } catch (e) {
        console.warn('[DuckDB] Failed to load parts_catalog.json — falling back to hardcoded lists', e);
      }
...
export async function getBestCombosForBlade(
  ...
  limit = 10,
  region?: Region
): Promise<{ ratchet: string; bit: string; assist: string | null; lockChip: string | null; overBlade: string | null; combo: string; raw_score: number; uses: number; first: number; second: number; third: number }[]> {
  const regionFilter = getRegionWhereClause(region);
  const rows = await query<{
    ratchet: string;
    bit: string;
    ...
    SELECT ratchet, bit, assist, lock_chip, over_blade, place, tournament_date::VARCHAR as tournament_date, stage
    FROM combo_usage
  ...
  const comboScores: Record<
    string,
    { ratchet: string; bit: string; assist: string | null; lockChip: string | null; overBlade: string | null; combo: string; raw_score: number; uses: number; first: number; second: number; third: number }
  > = {};
  const referenceDate = new Date();
  ...
  for (const row of rows) {
    const ratchet = normalizeRatchet(row.ratchet);
    const bit = normalizeBit(row.bit);
    const assist = row.assist || null;
    const lockChip = row.lock_chip || null;
    const overBlade = row.over_blade || null;
    const key = `${ratchet}|${bit}|${assist || ''}`;
    if (!comboScores[key]) {
      const parts = [ratchet, bit];
      if (assist) parts.splice(0, 0, assist);
      comboScores[key] = {
        ratchet: ratchet,
        bit: bit,
        assist: assist,
...
/**
 * Get best blade+ratchet+bit combinations for a specific assist blade.
 */
export async function getBestCombosForAssist(
  assistName: string,
  limit = 10,
  region?: Region
): Promise<{ blade: string; ratchet: string; bit: string; combo: string; raw_score: number; uses: number; first: number; second: number; third: number }[]> {
  const regionFilter = getRegionWhereClause(region);
  const rows = await query<{
    blade: string;
    ratchet: string;
    ...
    FROM combo_usage
    WHERE LOWER(assist) = LOWER('${assistName.replace(/'/g, "''")}')${regionFilter}
  `);

  const comboScores: Record<
    string,
    { blade: string; ratchet: string; bit: string; combo: string; raw_score: number; uses: number; first: number; second: number; third: number }
  > = {};
  const referenceDate = new Date();
  ...
  for (const row of rows) {
    ...
    if (!comboScores[key]) {
      comboScores[key] = {
        ...
        raw_score: 0, uses: 0, first: 0, second: 0, third: 0,
      };
    }
...
/**
 * Get best blade+ratchet+bit combinations for a specific lock chip.
 */
export async function getBestCombosForLockChip(
  lockChipName: string,
  limit = 10,
  region?: Region
): Promise<{ blade: string; ratchet: string; bit: string; assist: string | null; combo: string; raw_score: number; uses: number; first: number; second: number; third: number }[]> {
  const regionFilter = getRegionWhereClause(region);
  const rows = await query<{
    blade: string;
    ratchet: string;
    ...
    SELECT blade, ratchet, bit, assist, place, tournament_date::VARCHAR as tournament_date, stage
    FROM combo_usage
    WHERE LOWER(lock_chip) = LOWER('${lockChipName.replace(/'/g, "''")}')${regionFilter}
  `);

  const comboScores: Record<
    string,
    { blade: string; ratchet: string; bit: string; assist: string | null; combo: string; raw_score: number; uses: number; first: number; second: number; third: number }
  > = {};
  const referenceDate = new Date();
  ...
  for (const row of rows) {
    ...
    const bit = normalizeBit(row.bit);
    const assist = row.assist || null;
    const key = `${blade}|${assist || ''}|${ratchet}|${bit}`;
    if (!comboScores[key]) {
      const parts = [blade];
      if (assist) parts.push(assist);
      parts.push(ratchet, bit);
      comboScores[key] = {
...
/**
 * Get best blade+ratchet+bit combinations for a specific over blade.
 */
export async function getBestCombosForOverBlade(
  ...
  const rows = await query<{
    ...
    SELECT blade, ratchet, bit, place, tournament_date::VARCHAR as tournament_date, stage
    FROM combo_usage
    WHERE LOWER(over_blade) = LOWER('${overBladeName.replace(/'/g, "''")}')${regionFilter}
  `);
